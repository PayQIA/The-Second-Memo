<p align="center">
  <picture>
    ...
    <img src="apps/web/public/logo-fullmark.svg" alt="The Second Memo" width="400" />
  </picture>
</p>
...
<p align="center">
  <a href="https://the-second-memo.ai/docs">文档</a> ·
  <a href="https://the-second-memo.ai/docs/quickstart">快速开始</a> ·
  <a href="https://console.the-second-memo.ai">控制台</a> ·
  <a href="https://the-second-memo.link/discord">Discord</a>
</p>

<p align="center">
  <a href="https://www.npmjs.com/package/the-second-memo"><img src="https://img.shields.io/npm/v/the-second-memo?style=flat-square&color=blue" alt="npm" /></a>
  <a href="https://pypi.org/project/the-second-memo/"><img src="https://img.shields.io/pypi/v/the-second-memo?style=flat-square&color=blue" alt="pypi" /></a>
  <a href="https://the-second-memo.ai/docs"><img src="https://img.shields.io/badge/docs-the-second-memo.ai-blue?style=flat-square" alt="docs" /></a>
</p>
...
The Second Memo 是为 AI 设计的记忆与上下文层。在 **[LongMemEval](https://github.com/xiaowu0162/LongMemEval)、[LoCoMo](https://github.com/snap-research/locomo) 和 [ConvoMem](https://github.com/Salesforce/ConvoMem)** 这三大 AI 记忆基准上均位列第一。

我们是一个研究团队，围绕这个引擎打造插件和工具生态。

你的 AI 在对话之间会把一切忘干净，The Second Memo 就是来解决这个问题的。
...
直接用我们的应用，给自己搭一份专属的 the-second-memo。它会**在每次对话之间维护一张持久的记忆图谱**。
...
**[→ 跳到开发者快速开始](#用-the-second-memo-api-构建)**
...
The Second Memo 的应用、浏览器扩展、插件和 MCP 服务器，可以为任何兼容的 AI 助手提供持久记忆。装一次，AI 从此记住你。

### 应用

不用写代码，直接用我们面向消费者的应用——免费。

入口：https://app.the-second-memo.ai
...
The Second Memo 已经为 Claude Code、OpenCode、OpenClaw、Hermes 提供了开箱即用的插件。

<img width="844" height="484" alt="image" src="https://github.com/user-attachments/assets/ecb879a2-8652-495d-9228-f305a97ba603" />

这些插件本质上是 the-second-memo API 的实现，全部开源：

- Openclaw 插件：https://github.com/the-second-memoai/openclaw-the-second-memo
- Claude Code 插件：https://github.com/the-second-memoai/claude-the-second-memo
- OpenCode 插件：https://github.com/the-second-memoai/opencode-the-second-memo
- Hermes agent（The Second Memo 作为记忆 provider）：https://github.com/NousResearch/hermes-agent
...
更多 MCP 细节见：https://the-second-memo.ai/docs/the-second-memo-mcp/mcp
...
装好之后，The Second Memo 在后台运行：

1. **正常和你的 AI 对话。** 聊聊偏好、提提项目、讨论一下问题。
2. **The Second Memo 自动提取并存下关键信息。** 事实、偏好、项目上下文——杂音不留。
...
MCP 服务器开源——[查看源码](https://the-second-memo.ai/docs/the-second-memo-mcp/mcp)。
...
## 用 The Second Memo API 构建

如果你在做 AI agent 或应用，The Second Memo 用一套 API 给你完整的上下文栈——记忆、RAG、用户画像、连接器、文件处理，全包。

### 安装

```bash
npm install the-second-memo    # 或者：pip install the-second-memo
```
...
The Second Memo 会自动抽取记忆、构建用户画像、返回相关上下文。不用搭 embedding 流水线，不用配向量库，不用操心切分。
...
传统记忆方案依赖检索——你得先知道要问什么。The Second Memo 会自动为每个用户维护一份画像：
...
完整 API 文档 → [the-second-memo.ai/docs](https://the-second-memo.ai/docs)

---

## 基准测试

在各大 AI 记忆基准上，The Second Memo 都处于业界顶尖水平：
...
我们还做了 **[MemoryBench](https://the-second-memo.ai/docs/memorybench/overview)**——一个开源框架，用于对记忆服务做标准化、可复现的基准评测。The Second Memo、Mem0、Zep 等可以直接放在一起对比：
...
我们提供了一个 Agent skill，方便公司把自己的上下文/记忆方案拿来和 the-second-memo 对比。

```
npx skills add the-second-memoai/memorybench
```

跑完上面这条命令，再执行 `/benchmark-context`，The Second Memo 会自动帮你跑完整个流程。
...
**记忆不等于 RAG。** RAG 是检索文档片段——无状态，对所有人返回相同结果。记忆是**长期跟踪和提取关于用户的事实**。它知道「我刚搬到 SF」会覆盖「我住在 NYC」。The Second Memo 默认把两者一起跑，所以每次查询都能同时拿到知识库检索结果和个性化上下文。更多细节见：https://the-second-memo.ai/docs/concepts/memory-vs-rag

**自动遗忘。** The Second Memo 知道哪些记忆已经无关紧要。临时事实（「我明天有考试」）会在日期过后失效，冲突会自动消解，杂音不会沉淀成永久记忆。

---

## 链接

- 📖 [文档](https://the-second-memo.ai/docs)
- 🚀 [快速开始](https://the-second-memo.ai/docs/quickstart)
- 🧪 [MemoryBench](https://the-second-memo.ai/docs/memorybench/overview)
- 🔌 [集成](https://the-second-memo.ai/docs/integrations)
