...
const CHATGPT_LOG_PREFIX = "[the-second-memo:chatgpt]"
...
export function initializeChatGPT() {
	...
	setTimeout(() => {
		addThe Second MemoButtonToMemoriesDialog()
...
function setupChatGPTRouteChangeDetection() {
	...
	const checkForRouteChange = () => {
		if (window.location.href !== currentUrl) {
			...
			debugChatGPT("route changed, re-adding the-second-memo elements", currentUrl)
			setTimeout(() => {
				addThe Second MemoButtonToMemoriesDialog()
				addSaveChatGPTElementBeforeComposerBtn()
				setupChatGPTAutoFetch()
...
function addThe Second MemoButtonToMemoriesDialog() {
	...
	if (memoriesDialog.querySelector("#the-second-memo-save-button")) return
	...
	the-second-memoButton.className = "btn relative btn-primary-outline mr-2"
	...
	the-second-memoButton.addEventListener("click", async () => {
		await saveMemoriesToThe Second Memo()
	})
...
async function saveMemoriesToThe Second Memo() {
	try {
		...
		console.error("Error saving memories to the-second-memo:", error)
		DOMUtils.showToast("error")
	}
}
...
async function setupChatGPTAutoFetch() {
	...
	if (
		!promptTextarea ||
		promptTextarea.hasAttribute("data-the-second-memo-auto-fetch")
	) {
		return
	}

	promptTextarea.setAttribute("data-the-second-memo-auto-fetch", "true")

	const handleInput = () => {
		const content = promptTextarea.textContent?.trim() || ""
		syncAcceptedThe Second MemoState(promptTextarea)
		...
		chatGPTDebounceTimeout = setTimeout(async () => {
			if (hasAcceptedThe Second MemoContext(promptTextarea)) {
				clearMemorySuggestion("chatgpt", promptTextarea)
				return
			}
