...
import {
	...
	hasAcceptedThe Second MemoContext,
	setMemoryMarkerStatus,
	showLoadingSuggestion,
	showMarkerPopover,
	showMemorySuggestion,
	syncAcceptedThe Second MemoState,
...
const CLAUDE_LOG_PREFIX = "[the-second-memo:claude]"
...
export function initializeClaude() {
	...
	setTimeout(() => {
		addThe Second MemoButtonToClaudeMemoryDialog()
		addThe Second MemoIconToClaudeInput()
...
function setupClaudeRouteChangeDetection() {
	...
	const checkForRouteChange = () => {
		if (window.location.href !== currentUrl) {
			...
			debugClaude("route changed, re-adding the-second-memo icon", currentUrl)
			setTimeout(() => {
				addThe Second MemoButtonToClaudeMemoryDialog()
				addThe Second MemoIconToClaudeInput()
				setupClaudeAutoFetch()
...
function addThe Second MemoIconToClaudeInput() {
	...
	const the-second-memoIcon = createClaudeInputBarElement(async () => {
		await getRelatedMemoriesForClaude(
			POSTHOG_EVENT_KEY.CLAUDE_CHAT_MEMORIES_SEARCHED,
		)
	})

	the-second-memoIcon.id = `${ELEMENT_IDS.CLAUDE_INPUT_BAR_ELEMENT}-${Date.now()}-${Math.random().toString(36).substring(2, 11)}`

	if (anchorSlot?.parentElement === targetContainer) {
		targetContainer.insertBefore(the-second-memoIcon, anchorSlot)
	...
	targetContainer.appendChild(the-second-memoIcon)
...
async function getRelatedMemoriesForClaude(actionSource: string) {
	try {
		...
		const the-second-memoContainer = document.querySelector(
			'[data-the-second-memo-icon-added="true"]',
		)
		if (the-second-memoContainer?.parentElement?.previousElementSibling) {
			const pTag =
				the-second-memoContainer.parentElement.previousElementSibling.querySelector(
					"p",
				)
...
function getClaudeMemoryText(dialog: HTMLElement): string {
	...
	clonedDialog.querySelector("#the-second-memo-save-button")?.remove()
	...
	const memorySections = Array.from(
		...
		.filter((text) => {
			return (
				...
				!text.includes("Save to the-second-memo") &&
...
function addThe Second MemoButtonToClaudeMemoryDialog() {
	const memoryDialog = getClaudeMemoryDialog()
	if (!memoryDialog) return

	if (memoryDialog.querySelector("#the-second-memo-save-button")) return

	const the-second-memoButton = document.createElement("button")
	the-second-memoButton.id = "the-second-memo-save-button"

	const iconUrl = browser.runtime.getURL("/icon-16.png")

	the-second-memoButton.innerHTML = `
        <div style="display: inline-flex; align-items: center; justify-content: center; gap: 8px; white-space: nowrap;">
          <img src="${iconUrl}" alt="the-second-memo" style="width: 16px; height: 16px; flex-shrink: 0; border-radius: 2px;" />
          <span style="white-space: nowrap;">Save to the-second-memo</span>
        </div>
      `

	the-second-memoButton.style.cssText = `
	...
	the-second-memoButton.addEventListener("mouseenter", () => {
		the-second-memoButton.style.backgroundColor = "#2B2E33"
	})

	the-second-memoButton.addEventListener("mouseleave", () => {
		the-second-memoButton.style.backgroundColor = "#1C2026"
	})

	the-second-memoButton.addEventListener("click", async () => {
		await saveClaudeMemoriesToThe Second Memo(memoryDialog)
	})
	...
	if (heading?.parentElement) {
		heading.parentElement.insertBefore(the-second-memoButton, heading.nextSibling)
		return
	}

	memoryDialog.insertBefore(the-second-memoButton, memoryDialog.firstChild)
}

async function saveClaudeMemoriesToThe Second Memo(memoryDialog: HTMLElement) {
	try {
		...
		console.error("Error saving Claude memories to the-second-memo:", error)
		DOMUtils.showToast("error")
	}
}
...
function setupClaudePromptCapture() {
	...
	document.addEventListener(
		...
		async (event) => {
			...
			if (target.closest('[data-the-second-memo-connected-indicator="true"]')) {
				return
			}
...
async function setupClaudeAutoFetch() {
	...
	if (
		!textareaElement ||
		textareaElement.hasAttribute("data-the-second-memo-auto-fetch")
	) {
		return
	}

	textareaElement.setAttribute("data-the-second-memo-auto-fetch", "true")

	const handleInput = () => {
		const content = textareaElement.textContent?.trim() || ""
		syncAcceptedThe Second MemoState(textareaElement)
		...
		claudeDebounceTimeout = setTimeout(async () => {
			if (hasAcceptedThe Second MemoContext(textareaElement)) {
				clearMemorySuggestion("claude", textareaElement)
				return
			}
