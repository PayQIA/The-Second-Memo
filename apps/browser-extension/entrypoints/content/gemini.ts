...
const GEMINI_LOG_PREFIX = "[the-second-memo:gemini]"
...
export function initializeGemini() {
	...
	setTimeout(() => {
		addThe Second MemoIconToGeminiInput()
...
function addThe Second MemoIconToGeminiInput() {
	...
	const the-second-memoIcon = createGeminiInputBarElement(async () => {
		await getRelatedMemoriesForGemini(
			POSTHOG_EVENT_KEY.GEMINI_CHAT_MEMORIES_SEARCHED,
		)
	})

	the-second-memoIcon.id = `${ELEMENT_IDS.GEMINI_INPUT_BAR_ELEMENT}-${Date.now()}-${Math.random().toString(36).substring(2, 11)}`
...
async function setupGeminiAutoFetch() {
	...
	const input = getGeminiPromptInput()
	if (!input || input.hasAttribute("data-the-second-memo-auto-fetch")) {
		debugGemini("auto fetch skipped", {
			hasInput: !!input,
			alreadyAttached: input?.hasAttribute("data-the-second-memo-auto-fetch"),
		})
		return
	}

	input.setAttribute("data-the-second-memo-auto-fetch", "true")
	debugGemini("auto fetch attached", describeElement(input))

	const handleInput = () => {
		const content = getInputText(input).trim()
		syncAcceptedThe Second MemoState(input)
