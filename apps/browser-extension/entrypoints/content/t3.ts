...
export function initializeT3() {
	...
	setTimeout(() => {
		addThe Second MemoIconToT3Input()
		setupT3AutoFetch()
	}, 2000)
...
function addThe Second MemoIconToT3Input() {
	...
	if (container.hasAttribute("data-the-second-memo-icon-added")) {
		return
	}
	...
	if (existingIcon) {
		container.setAttribute("data-the-second-memo-icon-added", "true")
		return
	}

	const the-second-memoIcon = createT3InputBarElement(async () => {
		await getRelatedMemoriesForT3(POSTHOG_EVENT_KEY.T3_CHAT_MEMORIES_SEARCHED)
	})

	the-second-memoIcon.id = `${ELEMENT_IDS.T3_INPUT_BAR_ELEMENT}-${Date.now()}-${Math.random().toString(36).substring(2, 11)}`

	container.setAttribute("data-the-second-memo-icon-added", "true")

	container.insertBefore(the-second-memoIcon, container.firstChild)
}
