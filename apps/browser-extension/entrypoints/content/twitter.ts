...
async function loadSpaceGroteskFonts(): Promise<void> {
	if (document.getElementById("the-second-memo-modal-styles")) {
		return Promise.resolve()
	}

	const style = document.createElement("style")
	style.id = "the-second-memo-modal-styles"
	style.textContent = `
...
async function showOnboardingToast() {
	...
	if (!document.getElementById("the-second-memo-onboarding-toast-styles")) {
		const style = document.createElement("style")
		style.id = "the-second-memo-onboarding-toast-styles"
		style.textContent = `
	...
	description.textContent =
		"You can import all your Twitter bookmarks to The Second Memo with one click."
	...
	learnMoreButton.addEventListener("click", () => {
		window.open("https://docs.the-second-memo.ai/connectors/twitter", "_blank")
	})
...
function removeAllTwitterUI() {
	...
	document.querySelectorAll("[data-the-second-memo-button]").forEach((button) => {
		button.remove()
	})
}
...
function addButtonToElement(element: HTMLElement) {
	if (element.querySelector("[data-the-second-memo-button]")) {
		return
	}
	...
	button.setAttribute("data-the-second-memo-button", "true")
