/**
 * API service for the-second-memo browser extension
 */
...
async function makeAuthenticatedRequest<T>(
	...
	const response = await fetch(`${API_ENDPOINTS.SECONDMEMO_API}${endpoint}`, {
		...options,
		credentials: "omit",
		headers: {
...
/**
 * Save memory to The Second Memo API
 */
...
/**
 * Search memories using The Second Memo API
 */
...
/**
 * Save tweet to The Second Memo API (specific for Twitter imports)
 */
export async function saveAllTweets(
	...
	try {
		...
		if (error instanceof The Second MemoAPIError && error.statusCode === 409) {
