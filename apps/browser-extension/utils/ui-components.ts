...
export function createToast(state: ToastState): HTMLElement {
	...
	toast.id = ELEMENT_IDS.SECONDMEMO_TOAST
  ...
  `
	...
	if (!document.getElementById("the-second-memo-toast-styles")) {
		const style = document.createElement("style")
		style.id = "the-second-memo-toast-styles"
		style.textContent = `
      @font-face {
...
export function createConnectedIndicator(onClick: () => void): HTMLElement {
	...
	iconButton.setAttribute("aria-label", "the-second-memo connected")
	iconButton.dataset.the-second-memoConnectedIndicator = "true"
	iconButton.style.cssText = `
    display: inline-flex;
    align-items: center;
    justify-content: center;
  ...
  `
	...
	tooltip.textContent = "the-second-memo connected"
...
export function createT3InputBarElement(onClick: () => void): HTMLElement {
  ...
  `
	...
	const iconUrl = browser.runtime.getURL(iconFileName)
	iconButton.innerHTML = `
    <img src="${iconUrl}" width="20" height="20" alt="Get Related Memories from the-second-memo" style="border-radius: 4px;" />
  `
...
export function createProjectSelectionModal(
	...
	<div style="display: flex; flex-direction: column; gap: 8px;">
	    <h3 style="margin: 0; font-size: 16px; font-weight: 600; color: #ffffff; display: flex; align-items: center; gap: 8px;">
			<img src="${iconUrl}" width="20" height="20" alt="The Second Memo" style="border-radius: 4px;" />
			Import to The Second Memo
		</h3>
...
export const DOMUtils = {
	...
	showToast(
		...
		const existingToast = document.getElementById(ELEMENT_IDS.SECONDMEMO_TOAST)
