...
const API_BASE_URL = "https://api.the-second-memo.ai"

class The Second MemoAPIError extends Error {
	constructor(
		message: string,
		public status?: number,
	) {
		super(message)
		this.name = "The Second MemoAPIError"
	}
}
...
async function makeAuthenticatedRequest<T>(
	...
	try {
		...
		if (!response.ok) {
			...
			throw new The Second MemoAPIError(errorMessage, response.status)
		}

		if (!response.headers.get("content-type")?.includes("application/json")) {
			throw new The Second MemoAPIError("Invalid response format from API")
		}
		...
		if (
			...
			err instanceof The Second MemoAPIError
		) {
			throw err
		}

		// Handle network errors or other fetch errors
		throw new The Second MemoAPIError(
...
export async function addProject(request: AddProjectRequest): Promise<Project> {
	...
	await showToast({
		...
		title: "Project Added",
		message: "Successfully added project to The Second Memo",
	})
...
export async function addMemory(request: AddMemoryRequest): Promise<Memory> {
	try {
		...
		await showToast({
			style: Toast.Style.Success,
			title: "Memory Added",
			message: "Successfully added memory to The Second Memo",
		})
