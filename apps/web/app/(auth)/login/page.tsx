	...
	return (
		<div className={cn("max-w-sm text-center", className)}>
			<h1 className="text-balance text-lg font-medium leading-tight tracking-tight sm:text-xl lg:text-[1.375rem] xl:text-2xl">
				...
				<span className="block text-brand-accent">with the-second-memo</span>
			</h1>
			<p
				className={cn(
...
export default function LoginPage() {
	...
	useEffect(() => {
		const savedMethod = localStorage.getItem("the-second-memo-last-login-method")
	...
	function setPendingLoginMethod(method: string) {
		try {
			localStorage.setItem("the-second-memo-pending-login-method", method)
			localStorage.setItem(
				"the-second-memo-pending-login-timestamp",
				String(Date.now()),
			)
		} catch {}
	}
	...
	useEffect(() => {
		if (params.get("error")) {
			try {
				localStorage.removeItem("the-second-memo-pending-login-method")
				localStorage.removeItem("the-second-memo-pending-login-timestamp")
			} catch {}
		}
	}, [params])
								...
								{submittedEmail ? (
									...
									<div className="flex flex-col" style={{ gap: "12px" }}>
										...
										<div className="flex flex-col gap-3">
											{process.env.NEXT_PUBLIC_HOST_ID === "the-second-memo" ||
										...
										<div className="flex flex-col gap-6">
											...
											>
												...
												<span className="inline-block">
													<a
														className="underline"
														href="https://the-second-memo.ai/terms/"
													>
														Terms
													</a>{" "}
													and{" "}
													<a
														className="underline"
														href="https://the-second-memo.ai/privacy/"
													>
														Privacy Policy
													</a>
													.
