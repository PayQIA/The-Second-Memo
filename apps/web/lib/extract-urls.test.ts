...
describe("extractUrls", () => {
	...
	it("normalizes scheme-less URLs", () => {
		const { urls } = extractUrls("check the-second-memo.ai for details")
		expect(urls).toEqual(["https://the-second-memo.ai"])
	})
	...
	it("keeps real URLs while skipping emails in the same text", () => {
		const { urls } = extractUrls(
			"email john.doe@example.com or visit https://the-second-memo.ai",
		)
		expect(urls).toEqual(["https://the-second-memo.ai"])
	})
