...
const mcpMigrationSchema = z.object({
	url: z
		...
		.regex(
			/^https:\/\/mcp\.the-second-memo\.ai\/[^/]+\/sse$/,
			"Link must be in format: https://mcp.the-second-memo.ai/userId/sse",
		),
})
...
export function ConnectAIModal({
	...
	const extractUserIdFromMCPUrl = (url: string): string | null => {
		const regex = /^https:\/\/mcp\.the-second-memo\.ai\/([^/]+)\/sse$/
	...
	function getMcpServerUrl() {
		return "https://mcp.the-second-memo.ai/mcp"
	}
	...
	function getMcpConfigSnippet() {
		...
		if (selectedProject && selectedProject !== "none") {
			...
			if (projectIdForCommand) {
				config.mcpServers.the-second-memo.headers = {
	...
	function getCursorDeeplink() {
		return "cursor://anysphere.cursor-deeplink/mcp/install?name=the-second-memo&config=eyJ1cmwiOiJodHRwczovL2FwaS5zdXBlcm1lbW9yeS5haS9tY3AifQ%3D%3D"
	}
		...
		<Dialog onOpenChange={setIsOpen} open={isOpen}>
			...
			<DialogContent className="max-h-[calc(100dvh-1rem)] w-[calc(100vw-1rem)] max-w-[calc(100vw-1rem)] overflow-y-auto overflow-x-hidden p-4 sm:max-w-4xl sm:p-6">
				<DialogHeader className="pr-7 sm:pr-0">
					<DialogTitle>Connect the-second-memo to Your AI</DialogTitle>
					<DialogDescription>
											...
											<div className="space-y-4">
												<div className="flex flex-col items-center gap-4 rounded-lg border border-green-500/20 bg-green-500/5 p-6">
													<p className="text-center text-sm text-foreground/80">
														Open Cursor and add the-second-memo in one step, or
													...
													>
														<img
															alt="Add The Second Memo MCP server to Cursor"
												...
												<div className="space-y-3">
													...
													{createMcpApiKeyMutation.isPending ? (
														...
														<>
															...
															<p className="text-xs text-muted-foreground">
																Bearer token uses your the-second-memo API key.
															</p>
														</>
													)}
												</div>
					...
					<div className="gap-2 hidden">
						<div>
							...
							<p className="text-xs text-muted-foreground mt-2">
								Use this URL to configure the-second-memo in your AI assistant
							</p>
						</div>
					...
					<div className="flex flex-col gap-3 pt-4 sm:flex-row sm:items-center sm:justify-between">
						<div className="flex flex-wrap items-center gap-2 sm:gap-4">
							<Button
								onClick={() =>
									window.open(
										"https://docs.the-second-memo.ai/the-second-memo-mcp/introduction",
										"_blank",
									)
								}
								variant="outline"
									...
									<div className="flex flex-col gap-2">
											...
											{({ state, handleChange, handleBlur }) => (
												<>
													<Input
														...
														placeholder="https://mcp.the-second-memo.ai/your-user-id/sse"
														value={state.value}
													/>
										...
										<p className="text-xs text-muted-foreground">
											...
											<span className="font-mono">
												https://mcp.the-second-memo.ai/userId/sse
											</span>
										</p>
									</div>
								</div>
