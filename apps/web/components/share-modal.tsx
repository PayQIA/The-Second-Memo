...
export function ShareModal({
	...
	const handleDownload = useCallback(async () => {
		try {
			...
			a.href = url
			a.download = `the-second-memo-graph-${Date.now()}.png`
	...
	const handleShareTwitter = useCallback(async () => {
		const text = encodeURIComponent(
			"Check out my knowledge graph on the-second-memo! 🧠\n\nhttps://the-second-memo.ai",
		)
		window.open(`https://twitter.com/intent/tweet?text=${text}`, "_blank")
	}, [])

	const handleShareLinkedIn = useCallback(async () => {
		const url = encodeURIComponent("https://the-second-memo.ai")
		window.open(
				...
				<div className="flex flex-col gap-3 min-w-0">
					...
					<div className="grid grid-cols-[2rem_minmax(0,1fr)_2rem] items-center gap-2 px-1 sm:px-2">
						...
						<DialogHeader className="min-w-0">
							...
							>
								Share snapshot of your the-second-memo
							</DialogTitle>
						</DialogHeader>
