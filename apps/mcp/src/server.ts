...
import { The Second MemoClient } from "./client"
...
export class The Second MemoMCP extends McpAgent<Env, unknown, Props> {
		...
		this.server.registerPrompt(
			...
			async (args: ContextPromptArgs) => {
				try {
					...
					parts.push(
						"**Important:** Whenever the user shares informative facts, preferences, personal details, or any memory-worthy information, use the `memory` tool to save it to The Second Memo. This helps maintain context across conversations.",
					)
					parts.push("")
	...
	/**
	 * Get a The Second MemoClient instance configured with the API key
	 */
	private getClient(containerTag?: string): The Second MemoClient {
		if (!this.props) {
		...
		const apiUrl = this.env.API_URL || "https://api.the-second-memo.ai"
		return new The Second MemoClient(
			apiKey,
