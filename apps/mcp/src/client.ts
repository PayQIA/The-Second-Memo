...
export class The Second MemoClient {
	...
	constructor(
		bearerToken: string,
		containerTag?: string,
		apiUrl = "https://api.the-second-memo.ai",
	) {
	...
	private handleError(error: unknown): never {
		...
		if (
			...
			throw new Error("Request to The Second Memo API timed out")
		}
		...
		if (error && typeof error === "object" && "status" in error) {
			...
			switch (status) {
				...
				case 401:
					throw new Error("Authentication failed. Please re-authenticate.")
				case 402:
					throw new Error("Memory limit reached. Upgrade at the-second-memo.ai")
				case 403:
					throw new Error(
