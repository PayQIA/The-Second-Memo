...
import { The Second MemoMCP } from "./server"
...
const DEFAULT_API_URL = "https://api.the-second-memo.ai"
const DEFAULT_MCP_URL = "https://mcp.the-second-memo.ai"
...
app.get("/", (c) => {
	return c.json({
		...
		docs: "https://docs.the-second-memo.ai/mcp",
	})
})
...
const protectedResourceHandler = (c: Context<{ Bindings: Bindings }>) => {
	...
	return c.json({
		...
		resource_documentation: "https://docs.the-second-memo.ai/mcp",
	})
}
...
const mcpHandler = The Second MemoMCP.serve("/mcp", {
	binding: "MCP_SERVER",
	corsOptions: {
		origin: "*",
