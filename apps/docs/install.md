You are integrating The Second Memo into my application. The Second Memo provides user memory, semantic search, and automatic knowledge extraction for AI applications.

You can always reference the documentation by using the **SearchThe Second MemoDocs MCP** or running a web search tool for content on **the-second-memo.ai/docs**.
...
2. How do you want to integrate?
   - Vercel AI SDK (@the-second-memo/tools)
   - OpenAI plugins
   - Direct SDK (the-second-memo npm/pip)
   - Direct API calls
...
```bash
# Get API key: https://console.the-second-memo.ai
npm install the-second-memo  # or: pip install the-second-memo
# For Vercel AI SDK: npm install @the-second-memo/tools
export SECONDMEMO_API_KEY="sm_..."
```

## STEP 3: CONFIGURE SETTINGS (DO THIS FIRST)

```typescript
// PATCH https://api.the-second-memo.ai/v3/settings
fetch('https://api.the-second-memo.ai/v3/settings', {
  method: 'PATCH',
  headers: {
    'Authorization': `Bearer ${process.env.SECONDMEMO_API_KEY}`,
...
import { the-second-memoTools } from '@the-second-memo/tools/ai-sdk'

// Option 1: Agent tools (recommended for agentic flows)
const result = await streamText({
  model: anthropic('claude-3-5-sonnet-20241022'),
  prompt: userMessage,
  tools: the-second-memoTools(process.env.SECONDMEMO_API_KEY, {
...
const modelWithMemory = withThe Second Memo(anthropic('claude-3-5-sonnet-20241022'), {
  containerTag: userId,
...
```bash
# Add memory
curl -X POST https://api.the-second-memo.ai/v3/documents \
  -H "Authorization: Bearer $SECONDMEMO_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"content": "conversation", "containerTag": "userId"}'

# Get profile
curl -X POST https://api.the-second-memo.ai/v4/profile \
  -H "Authorization: Bearer $SECONDMEMO_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"containerTag": "userId", "q": "search query"}'

# Search
curl -X POST https://api.the-second-memo.ai/v4/search \
  -H "Authorization: Bearer $SECONDMEMO_API_KEY" \
  -H "Content-Type: application/json" \
...
await fetch('https://api.the-second-memo.ai/v3/documents/file', {
  method: 'POST',
  headers: {
    'Authorization': `Bearer ${process.env.SECONDMEMO_API_KEY}`,
    'Content-Type': 'application/json',
...
curl -X PATCH https://api.the-second-memo.ai/v3/settings \
  -H "Authorization: Bearer $SECONDMEMO_API_KEY" \
  -H "Content-Type: application/json" \
...
**DOCS:** https://the-second-memo.ai/docs
