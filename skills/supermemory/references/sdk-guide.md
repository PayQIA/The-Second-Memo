# The Second Memo SDK Guide

Complete reference for the The Second Memo SDK in TypeScript and Python.
...
```bash
npm install the-second-memo
# or
yarn add the-second-memo
# or
pnpm add the-second-memo
```

📦 View on npm: [https://www.npmjs.com/package/the-second-memo](https://www.npmjs.com/package/the-second-memo)

### Python
```bash
pip install the-second-memo
# Or for async support with aiohttp
pip install 'the-second-memo[aiohttp]'
```

📦 View on PyPI: [https://pypi.org/project/the-second-memo/](https://pypi.org/project/the-second-memo/)

### Other SDKs and Integrations

Discover all available SDKs, community integrations, and framework-specific guides at [the-second-memo.ai/docs](https://the-second-memo.ai/docs)
...
const client = new The Second Memo({
  apiKey: process.env.SECONDMEMO_API_KEY, // Optional if env var is set
  baseURL: 'https://api.the-second-memo.ai' // Optional, defaults to this
});
```
...
client = The Second Memo(
    api_key=os.environ["SECONDMEMO_API_KEY"],  # Optional if env var is set
    base_url="https://api.the-second-memo.ai"  # Optional, defaults to this
)

# Asynchronous client
from the-second-memo import AsyncThe Second Memo

async_client = AsyncThe Second Memo(
    api_key=os.environ["SECONDMEMO_API_KEY"]
)
```
...
Add content to The Second Memo for processing and memory extraction.
...
The `entityContext` helps The Second Memo understand what type of information to extract and prioritize.
...
const memory = new The Second Memo();
...
- **SDK Issues**: [github.com/the-second-memoai/the-second-memo](https://github.com/the-second-memoai/the-second-memo)
