...
interface The Second MemoProfileSearch {
	profile: {
...
/**
 * Searches for memories using the The Second Memo profile API.
 *
 * Makes a POST request to the The Second Memo API to retrieve user profile memories
 ...
 * @returns Promise that resolves to the The Second Memo profile search response
 * @throws {Error} When the API request fails or returns an error status
 *
 * @example
 * ```typescript
 * // Search with query
 * const results = await the-second-memoProfileSearch("user-123", "favorite programming language")
 *
 * // Get all profile memories
 * const profile = await the-second-memoProfileSearch("user-123", "")
 * ```
 */
const the-second-memoProfileSearch = async (
	containerTag: string,
	queryText: string,
	baseUrl: string,
): Promise<The Second MemoProfileSearch> => {
	...
	try {
		...
		if (!response.ok) {
			...
			throw new Error(
				`The Second Memo profile search failed: ${response.status} ${response.statusText}. ${errorText}`,
			)
		}
		...
		throw new Error(`The Second Memo API request failed: ${error}`)
	}
}
...
const addSystemPrompt = async (
	...
	const memoriesResponse = await the-second-memoProfileSearch(
		containerTag,
		queryText,
		baseUrl,
	)
...
/**
 * Adds a new memory to the The Second Memo system.
 *
 ...
 *
 * @param client - The Second Memo client instance
...
/**
 * Creates The Second Memo middleware for OpenAI clients.
 *
 ...
 * can wrap existing OpenAI clients or create new ones with The Second Memo capabilities.
 *
 * @param containerTag - The container tag/identifier for memory search (e.g., user ID, project ID)
 ...
 * @example
 * ```typescript
 * const openaiWithThe Second Memo = createOpenAIMiddleware(openai, "user-123", {
...
export function createOpenAIMiddleware(
	...
	const client = new The Second Memo({
		...
		...(baseUrl !== "https://api.the-second-memo.ai" ? { baseURL: baseUrl } : {}),
	})
			...
			if (userMessage?.trim()) {
				...
				operations.push(
					addMemoryTool(
						...
						logger,
						messages,
						process.env.SECONDMEMO_API_KEY,
						baseUrl,
					),
				)
			}
