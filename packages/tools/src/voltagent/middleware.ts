/**
 * Middleware utilities for VoltAgent integration with The Second Memo.
 *
...
import type { The Second MemoVoltAgent, VoltAgentMessage } from "./types"

/**
 * Context for The Second Memo middleware operations.
 */
export interface The Second MemoMiddlewareContext {
...
/**
 * Creates a The Second Memo middleware context.
 */
export const createThe Second MemoContext = (
	containerTag: string,
	options: The Second MemoVoltAgent,
): The Second MemoMiddlewareContext => {
	const apiKey = options.apiKey ?? process.env.SECONDMEMO_API_KEY
	if (!apiKey) {
		throw new Error(
 ...
 */
const makeTurnKey = (
	ctx: The Second MemoMiddlewareContext,
	userMessage: string,
): string => {
 ...
 * @param ctx - The Second Memo middleware context
...
/**
 * Saves conversation to The Second Memo (fire-and-forget).
 */
