...
interface WrapVercelLanguageModelOptions {
	...
	/** The Second Memo API key (falls back to SECONDMEMO_API_KEY env var) */
	apiKey?: string
	/** Custom The Second Memo API base URL */
	baseUrl?: string
	/**
	...
	/**
	 * When The Second Memo memory retrieval / injection fails or times out:
...
/**
 * Wraps a language model with the-second-memo middleware to automatically inject relevant memories
 * into the system prompt based on the user's message content.
 *
 * This wrapper searches the the-second-memo API for relevant memories using the container tag
 ...
 *
 * @param model - The language model to wrap with the-second-memo capabilities (V2 or V3)
 * @param options - Configuration options for The Second Memo integration
 ...
 * @param options.apiKey - Optional The Second Memo API key to use instead of the environment variable
 * @param options.baseUrl - Optional base URL for the The Second Memo API (default: "https://api.the-second-memo.ai")
 ...
 * @throws {Error} When the-second-memo memory retrieval fails and `skipMemoryOnError` is `false`
 */
const wrapVercelLanguageModel = <T extends LanguageModel>(
	model: T,
	options: WrapVercelLanguageModelOptions,
): T => {
	const providedApiKey = options.apiKey ?? process.env.SECONDMEMO_API_KEY
	...
	const ctx = createThe Second MemoContext({
				...
				return async (params: LanguageModelCallOptions) => {
					...
					try {
						...
						if (skipMemoryOnError) {
							ctx.logger.warn(
								"The Second Memo retrieval failed; continuing without injected memories",
								{
									error:
...
export {
	wrapVercelLanguageModel as withThe Second Memo,
	type WrapVercelLanguageModelOptions as WithThe Second MemoOptions,
	type PromptTemplate,
