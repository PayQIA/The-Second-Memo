...
// Mock the The Second Memo SDK (same pattern as claude-memory.test.ts) so tool
...
vi.mock("the-second-memo", () => {
	return {
		default: class MockThe Second Memo {
...
describe("memoryForget", () => {
	...
	it("issues DELETE /v4/memories with the forget payload", async () => {
		...
		expect(url).toBe("https://api.the-second-memo.ai/v4/memories")
