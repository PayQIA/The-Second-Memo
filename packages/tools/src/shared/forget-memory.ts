const DEFAULT_BASE_URL = "https://api.the-second-memo.ai"
 ...
 *
 * The the-second-memo SDK version this package depends on (v3) has no
...
export async function forgetMemoryRequest(
	...
	if (!response.ok) {
		...
		throw new Error(
			`The Second Memo forget memory failed: ${response.status} ${response.statusText}. ${errorText}`,
		)
	}
}
