/**
 * Integration tests for the withThe Second Memo wrapper
 */
...
const shouldRunIntegration = !!process.env.SECONDMEMO_API_KEY
...
describe.skipIf(!shouldRunIntegration)(
	"Integration: withThe Second Memo wrapper with real API",
	() => {
		describe("doGenerate flow", () => {
			it("should fetch real memories and inject into params passed to model", async () => {
				...
				const wrapped = withThe Second Memo(model, {
