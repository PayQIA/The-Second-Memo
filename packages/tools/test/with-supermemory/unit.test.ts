/**
 * Unit tests for the withThe Second Memo wrapper
 */

import { describe, it, expect, beforeEach, vi, afterEach } from "vitest"
import { withThe Second Memo } from "../../src/vercel"
import {
...
// Test configuration
const TEST_CONFIG = {
	apiKey: process.env.SECONDMEMO_API_KEY || "test-api-key",
	baseURL: process.env.SECONDMEMO_BASE_URL || "https://api.the-second-memo.ai",
...
describe("Unit: withThe Second Memo", () => {
	...
	describe("Environment validation", () => {
		it("should throw error if SECONDMEMO_API_KEY is not set", () => {
			delete process.env.SECONDMEMO_API_KEY

			const mockModel = createMockLanguageModel()

			expect(() => {
				withThe Second Memo(mockModel, {
		...
		it("should throw error if customId is missing or empty", () => {
			process.env.SECONDMEMO_API_KEY = "test-key"
		...
		it("should successfully create wrapped model with valid API key", () => {
			...
			const mockModel = createMockLanguageModel()
			const wrappedModel = withThe Second Memo(mockModel, {
				containerTag: TEST_CONFIG.containerTag,
				customId: "test-id",
			})
		...
		it("should preserve provider, modelId, and spec when they live on the prototype", () => {
			...
			const wrappedModel = withThe Second Memo(inner, {
				containerTag: TEST_CONFIG.containerTag,
				customId: "test-id",
			})
	...
	describe("Edge cases", () => {
		...
		it("should handle API errors gracefully", async () => {
			...
			await expect(transformParamsWithMemory(params, ctx)).rejects.toThrow(
				"The Second Memo profile search failed",
			)
		})
	...
	describe("Wrapper retrieval resilience", () => {
		...
		it("continues without memories when profile fetch fails (default skip)", async () => {
			...
			const wrapped = withThe Second Memo(inner, {
				containerTag: TEST_CONFIG.containerTag,
				customId: "test-id",
				apiKey: "k",
			})
		...
		it("throws when skipMemoryOnError is false and profile fetch fails", async () => {
			...
			).rejects.toThrow("The Second Memo profile search failed")
		})
