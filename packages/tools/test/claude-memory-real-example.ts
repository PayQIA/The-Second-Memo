...
export async function handleClaudeMemoryToolCall(
	...
	const memoryTool = createClaudeMemoryTool(the-second-memoApiKey, {
...
export async function realClaudeMemoryExample() {
	...
	const SECONDMEMO_API_KEY = process.env.SECONDMEMO_API_KEY

	if (!ANTHROPIC_API_KEY || !SECONDMEMO_API_KEY) {
		console.error("❌ Missing API keys:")
		console.error("- Set ANTHROPIC_API_KEY for Claude")
		console.error("- Set SECONDMEMO_API_KEY for The Second Memo")
		return
	}
	...
	if (responseData.content) {
		...
		const results = await Promise.all(
			memoryToolCalls.map((block: any) => {
				...
				return handleClaudeMemoryToolCall(block, SECONDMEMO_API_KEY, {
...
export async function processClaudeResponse(
	...
	if (claudeResponseData.content) {
		...
		const results = await Promise.all(
			memoryToolCalls.map((block: any) =>
				handleClaudeMemoryToolCall(block, the-second-memoApiKey, config),
			),
		)
...
export async function testWithRealToolCall() {
	...
	if (!process.env.SECONDMEMO_API_KEY) {
		console.error("❌ SECONDMEMO_API_KEY required for testing")
		return
	}
...
export async function runRealExamples() {
	...
	if (process.env.ANTHROPIC_API_KEY && process.env.SECONDMEMO_API_KEY) {
		console.log(`\\n${"=".repeat(70)}\\n`)
		await realClaudeMemoryExample()
	} else {
		console.log(
			"\\n⚠️  Set ANTHROPIC_API_KEY and SECONDMEMO_API_KEY to run full API example",
		)
	}
}
