...
// Install: npm install @anthropic-ai/sdk @the-second-memo/tools
...
# Test the memory tool using cURL commands against your the-second-memo API
...
curl -X POST "https://api.the-second-memo.ai/v3/search" \\
  -H "Authorization: Bearer YOUR_API_KEY" \\
  -H "Content-Type: application/json" \\
  -d '{
...
curl -X PATCH "https://api.the-second-memo.ai/v3/documents/DOCUMENT_ID" \\
  -H "Authorization: Bearer YOUR_API_KEY" \\
  -H "Content-Type: application/json" \\
  -d '{
...
curl -X DELETE "https://api.the-second-memo.ai/v3/documents/DOCUMENT_ID" \\
  -H "Authorization: Bearer YOUR_API_KEY"
`
...
export async function runAllExamples() {
	if (!process.env.SECONDMEMO_API_KEY) {
		console.error("❌ SECONDMEMO_API_KEY environment variable is required")
