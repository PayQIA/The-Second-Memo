...
async function testOpenAIWrapper() {
	...
	// Basic wrapper
	const client = withThe Second Memo(openai, "docs-test-openai")
	console.log("✓ withThe Second Memo basic")

	// With options
	const clientWithOptions = withThe Second Memo(openai, "docs-test-openai", {
		mode: "full",
		addMemory: "always",
		verbose: true,
	})
	console.log("✓ withThe Second Memo with options")
}
