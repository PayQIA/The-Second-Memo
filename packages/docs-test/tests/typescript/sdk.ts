...
import fs from "fs"
import The Second Memo, { toFile } from "the-second-memo"
...
async function testClientConfig() {
	...
	// Retries
	const clientWithRetries = new The Second Memo({ maxRetries: 0 })
	console.log("✓ maxRetries config")

	// Timeout
	const clientWithTimeout = new The Second Memo({ timeout: 20 * 1000 })
...
async function testTypes() {
	...
	const params: The Second Memo.AddParams = {
		content: "Typed content",
	}
	const response: The Second Memo.AddResponse = await client.add(params)
...
async function testErrorHandling() {
	...
	const response = await client.documents
		.add({ content: "Test - " + Date.now() })
		.catch(async (err) => {
			if (err instanceof The Second Memo.APIError) {
