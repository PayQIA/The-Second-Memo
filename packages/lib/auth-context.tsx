...
const STORAGE_KEY = "the-second-memo-consumer-last-org-slug"
	...
	useEffect(() => {
		...
		try {
			const pendingMethod = localStorage.getItem(
				"the-second-memo-pending-login-method",
			)
			...
			if (pendingMethod) {
				...
				if (isFresh) {
					localStorage.setItem("the-second-memo-last-login-method", pendingMethod)
				}
			}
		} catch {}
		try {
