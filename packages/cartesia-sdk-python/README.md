# The Second Memo Cartesia SDK

Memory-enhanced voice agents with [The Second Memo](https://the-second-memo.ai) and [Cartesia Line](https://cartesia.ai/agents).

## Installation

```bash
pip install the-second-memo-cartesia
```
...
from the-second-memo_cartesia import The Second MemoCartesiaAgent
...
async def get_agent(env, call_request):
    ...
    # Wrap with The Second Memo
    memory_agent = The Second MemoCartesiaAgent(
        agent=base_agent,
        api_key=os.getenv("SECONDMEMO_API_KEY"),
...
| `api_key`       | str          | No       | The Second Memo API key (or set `SECONDMEMO_API_KEY` env var)         |
...
memory_agent = The Second MemoCartesiaAgent(
    ...
    config=The Second MemoCartesiaAgent.MemoryConfig(
...
read_only_agent = The Second MemoCartesiaAgent(
    agent=base_agent,
    container_tag="user-123",
    custom_id="conversation-456",
...
2. **Retrieves memories** - Queries The Second Memo `/v4/profile` API with user's message
3. **Enriches context** - Adds memories to event history as system message
4. **Stores messages** - Sends conversation to The Second Memo (background, non-blocking)
5. **Passes to agent** - Forwards enriched event to wrapped LlmAgent

### What Gets Stored

User and assistant messages are sent to The Second Memo:
...
```bash
# Clone repository
git clone https://github.com/the-second-memoai/the-second-memo
cd the-second-memo/packages/cartesia-sdk-python
